...
KNIGHTS = {
            ...
            "mordred": Knights(
               ...
               power=30,
               hp=90,
               armours=[Armour("breastplate", 10),
                        Armour("boots",15)],
               weapon=Weapon("Poisoned Sword", 60),
               potion=Potion("Berserk",effect={"power": 15, "hp": -5, "protection": 10})
           ),
            "arthur": Knights(
               "Arthur",
               power=45,
...
def battle(KNIGHTS: dict) -> dict:
    result = {}
    result.update(KNIGHTS["lancelot"].battle_khights(KNIGHTS["mordred"]))
    result.update(KNIGHTS["arthur"].battle_khights(KNIGHTS["red knight"]))
    return result
