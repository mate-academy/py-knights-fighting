...
class Knight:
    def __init__(self,
        ...
        if isinstance(potion, dict) and "effect" in potion and "power" in potion["effect"]:
        ...
        if isinstance(potion, dict) and "effect" in potion and "protection" in potion["effect"]:
            self.armour += potion["effect"]["protection"]

        # Расчет очков здоровья с учетом зелья
        self.hp = hp
        if isinstance(potion, dict) and "effect" in potion and "hp" in potion["effect"]:
    ...
    def greetings(self) -> None:
        print(f"Welcome knight {self.name} to our tournament.\n",
              f"{self.name} will perform with the following characteristics:\n",
              f"\t\t hp - {self.hp}\n",
              f"\t\t power - {self.power}\n",
...
if __name__ == "__main__":
    ...
    for knight in KNIGHTS:
        knights.append(Knight(**KNIGHTS[knight]))
